...
def predict_basketball(h2h, total, hc, win):
    ...
    if hc:
        if total:
            if win:
                return {'handicap': format_float(hc), 'total': format_float(total), 'win': win}
            return {'handicap': format_float(hc), 'total': format_float(total)}
        return {'handicap': format_float(hc)}
    if total:
        if win:
